...
EPUBJS.Book.prototype.displayChapter = function(chap, end, deferred){
    ...
    if (chapter.mediaOverlay !== '') {
        chapter.mediaOverlayURI = book.manifest[chapter.mediaOverlay].url;
    }
...
EPUBJS.Chapter = function(spineObject, store, credentials){
    ...
    this.mediaOverlay = spineObject.mediaOverlay;
    this.mediaOverlayURI = null;
...
EPUBJS.Chapter.prototype.unload = function(store){
    ...
    if(this.tempUrl && store) {
        ...
        this.tempUrl = false;
        if (this.currentChapter.mediaOverlay) {
            var _URL = window.URL || window.webkitURL || window.mozURL;
            _URL.revokeObjectURL(this.currentChapter.mediaOverlayURI);
        }
    }
};
